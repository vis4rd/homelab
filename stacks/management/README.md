...
Newt (and Pangolin) are used for external access to some selected services. The two endpoints are
connected via a secure tunnel, protected by auth layer hosted on a remote server.
...
are participants in Newt's docker network can be accessed by the Pangolin server.
